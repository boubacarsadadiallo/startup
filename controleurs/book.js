  ...
  Book.update(req.body, {
    ...
    .then((num) => {
      if (num == 1) {
        ...
        res.send({
          message: `Cannot update Book with id=${id}. Book may not be found or req.body is empty!`,
        })
      }
    })
    .catch((err) => {
  ...
  Book.destroy({
    ...
    .then((num) => {
      if (num == 1) {
        ...
        res.send({
          message: `Cannot delete Book with id=${id}.Book may not be found!`,
        })
      }
    })
    .catch((err) => {
...
const deleteAllBooks = (req, res) => {
  Book.destroy({
    ...
    .then((nums) => {
      res.send({ message: `${nums} Books have been deleted successfully!` })
    })
    .catch((err) => {
      res.status(500).send({
